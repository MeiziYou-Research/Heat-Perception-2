    ...
    return `
      <section id="scene-driver-ranking" class="driver-ranking-section">
        <div class="driver-ranking-shell">
          ...
          <h2 class="driver-ranking-title">Heat perception - Associated factors</h2>
...
<div class="driver-bar-list">${rows}</div>
  ...
  function ensure() {
    ...
    /* Once rendered, keep the same DOM node so scroll anchoring stays stable. */
    const existingWrapper = document.getElementById("scene5-driver-ranking-wrapper");
    const existingSection = document.getElementById("scene-driver-ranking");
    if (existingWrapper || existingSection) return true;
  ...
  function boot() {
    if (window.__XLP_SCENE5_DRIVER_RANKING_BOOTED__) return;
    window.__XLP_SCENE5_DRIVER_RANKING_BOOTED__ = true;
