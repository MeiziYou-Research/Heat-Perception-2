...
(function () {
  ...
  let currentGroupKey = "emotion";
  let currentVariable = EMOTIONS.find(v => v.key === "Sadness") || EMOTIONS[0];
  let selectedFeature = null;

  document.addEventListener("DOMContentLoaded", initSafeSecondScene);

  async function initSafeSecondScene() {
    const mapEl = document.getElementById("global-map");
    if (!mapEl) {
      console.warn("No #global-map found. Safe second scene skipped.");
      return;
    }

    showStatus("Loading city data...");

    if (!window.mapboxgl) {
      showStatus("Mapbox GL JS is not loaded. Check CDN or internet connection.", true);
      return;
    }

    if (!window.echarts) {
      showStatus("ECharts is not loaded. Check CDN or internet connection.", true);
      return;
    }

    if (!CONFIG.mapboxToken || CONFIG.mapboxToken.includes("PASTE_YOUR")) {
      showStatus("Mapbox token missing. Please replace token in scene2-safe.js.", true);
      return;
    }
    ...
    try {
      ...
      console.error(err);
      showStatus("Failed to initialize second scene: " + err.message, true);
    }
  }

  async 
async function loadCityData() {
  const data = EMBEDDED_CITY_DATA;

  if (!data || !data.features || !Array.isArray(data.features)) {
    throw new Error("Embedded GeoJSON does not contain a valid features array.");
  }

  const fixedFeatures = data.features
    .map((feature, idx) => normalizeFeature(feature, idx))
    .filter(Boolean);

  if (!fixedFeatures.length) {
    throw new Error("No valid point features found in embedded GeoJSON.");
  }
  ...
  function normalizeFeature(feature, idx) {
    ...
    if (!geometry || geometry.type !== "Point" || !geometry.coordinates) {
      const lon = toNumber(p.LONGITUDE || p.longitude || p.lon || p.lng);
      const lat = toNumber(p.LATITUDE || p.latitude || p.lat);
    ...
    return {
      ...
      properties: {
        ...p,
        __id: idx,
        __city: getCityName(p),
        __country: getCountryName(p)
      }
    };
  }

  function setupControls() {
    const groupSelect = document.getElementById("safe-group-select");
    const variableSelect = document.getElementById("safe-variable-select");

    if (!groupSelect || !variableSelect) return;

    groupSelect.innerHTML = Object.entries(GROUPS)
      .map(([key, group]) => `<option value="${key}">${group.label}</option>`)
      .join("");

    groupSelect.value = currentGroupKey;

    groupSelect.addEventListener("change", () => {
      currentGroupKey = groupSelect.value;
      const variables = getAvailableVariables(currentGroupKey);
      currentVariable = variables[0] || currentVariable;
    ...
    variableSelect.addEventListener("change", () => {
      const variables = getAvailableVariables(currentGroupKey);
      currentVariable = variables.find(v => v.key === variableSelect.value) || variables[0];
      updateEverything();
    });
  ...
  function getAvailableVariables(groupKey) {
    const group = GROUPS[groupKey] || GROUPS.emotion;
    const variables = group.variables.filter(v => cityData.features.some(f => Number.isFinite(getValue(f.properties, v))));

    return variables.length ? variables : group.variables;
  }

  function setupMiniChart() {
    const el = document.getElementById("safe-mini-chart");
    if (el) {
  ...
  function setupMap() {
    ...
    map.on("style.load", () => {
      try {
        ...
        console.warn("Fog skipped", e);
      }
    });

    map.on("load", () => {
      addCityLayers();
      selectedFeature = pickInitialFeature();
      updateEverything(false);
      hideStatus();

      // LAYOUT_FIX_FORCE_RESIZE
      setTimeout(() => {
        if (map) map.resize();
        if (miniChart) miniChart.resize();
      }, 300);
    ...
    map.on("error", (e) => {
      ...
      showStatus("Mapbox error. Open browser Console for details.", true);
    });
  }

  function addCityLayers() {
    if (map.getSource("safe-cities")) {
      map.getSource("safe-cities").setData(cityData);
      return;
    }

    map.addSource("safe-cities", {
      type: "geojson",
      data: cityData
    });
    ...
    map.addLayer({
      id: "safe-city-selected",
      type: "circle",
      source: "safe-cities",
      filter: ["==", ["get", "__id"], -9999],
      paint: {
        "circle-radius": 16,
        "circle-color": "rgba(255,255,255,0)",
        "circle-stroke-width": 3,
        "circle-stroke-color": "#f2a06b",
        "circle-opacity": 1
      }
    });

    map.on("mouseenter", "safe-city-points", () => {
      map.getCanvas().style.cursor = "pointer";
    });

    map.on("mouseleave", "safe-city-points", () => {
      map.getCanvas().style.cursor = "";
    });

    map.on("click", "safe-city-points", (e) => {
  ...
  function updateEverything(shouldFly) {
    if (!selectedFeature) selectedFeature = pickInitialFeature();

    updateMapPaint();
    updateOverlay();
    drawMiniDistributionChart();

    if (shouldFly && map && selectedFeature.geometry) {
      map.flyTo({
  ...
  function updateMapPaint() {
    if (!map || !map.getLayer("safe-city-points")) return;

    map.setPaintProperty("safe-city-points", "circle-color", getColorExpression());
    map.setPaintProperty("safe-city-glow", "circle-color", getColorExpression());

    map.setPaintProperty("safe-city-points", "circle-radius", getRadiusExpression());
    map.setPaintProperty("safe-city-glow", "circle-radius", getRadiusExpression());

    if (selectedFeature) {
      map.setFilter("safe-city-selected", ["==", ["get", "__id"], Number(selectedFeature.properties.__id)]);
    }
  }
  ...
  function updateOverlay() {
    ...
    const value = getValue(p, currentVariable);
    const stats = getStats(currentVariable);
    const percentile = getPercentile(value, stats.values);

    setText("safe-variable-title", currentVariable.label);
    setText("safe-variable-value", formatValue(value));
    setText("safe-variable-unit", currentVariable.unit || "");
    setText("safe-city-name", getCityName(p));
    setText("safe-percentile-label", `${Math.round(percentile * 100)}TH PERCENTILE`);
    setText("safe-legend-min", formatValue(stats.min) + (currentVariable.unit || ""));
    setText("safe-legend-max", formatValue(stats.max) + (currentVariable.unit || ""));
  }

  function drawMiniDistributionChart() {
    if (!miniChart || !selectedFeature) return;

    const stats = getStats(currentVariable);
    const values = stats.values;
    const selectedValue = getValue(selectedFeature.properties, currentVariable);

    if (!values.length || !Number.isFinite(selectedValue)) return;
    ...
    const selectedY = Math.max(bins[selectedBinIndex].count, smoothCounts[selectedBinIndex], 1);
    ...
    miniChart.setOption({
      ...
      xAxis: {
        ...
        min: minVal,
        max: maxVal,
        axisLine: { show: true, lineStyle: { color: "rgba(220,238,248,0.30)" } },
        axisTick: { show: false },
      ...
      series: [
        ...
        },
        {
          ...
          smooth: true,
          symbol: "none",
          data: bins.map((b, i) => [b.center, smoothCounts[i]]),
          lineStyle: {
            width: 2.4,
            color: "#ed69ad"
          },
  ...
  function pickInitialFeature() {
    const arlington = cityData.features.find(f => String(getCityName(f.properties)).toLowerCase().includes("arlington"));
    if (arlington && Number.isFinite(getValue(arlington.properties, currentVariable))) return arlington;

    return cityData.features.find(f => Number.isFinite(getValue(f.properties, currentVariable))) || cityData.features[0];
  }
  ...
  function getColorExpression() {
    ...
    return [
      "interpolate",
      ["linear"],
      ["to-number", ["get", currentVariable.key]],
      min, "#56a9f2",
      mid, "#8f63ff",
      max, "#ed69ad"
    ];
  ...
  function getStats(variable) {
    const values = cityData.features
      .map(f => getValue(f.properties, variable))
  ...
  function getValue(properties, variable) {
    if (!properties || !variable) return NaN;
    return toNumber(properties[variable.key]);
  }
  ...
  function showStatus(message, isError) {
    const el = document.getElementById("safe-map-status");
    if (!el) return;
    ...
    if (isError) {
      el.style.background = "rgba(174, 43, 75, 0.86)";
    }
  }
